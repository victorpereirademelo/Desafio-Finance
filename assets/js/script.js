...
function finance() {
    ...
    const inFilter = data.filter(element => {
        if (element.type === 'IN') {
            return true;
        }
    });
    ...
    let inTotal = 0;
    inFilter.forEach(element => {
        inTotal += element.amount * element.price;
    });

    const inHtml = document.getElementById('in');
    let htmlIn = `Total de Entradas: ${inTotal.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' })}`;
    inHtml.innerHTML = htmlIn;

    const outFilter = data.filter(element => {
        if (element.type === 'OUT') {
            return true;
        }
    });
    ...
    let outTotal = 0;
    outFilter.forEach(element => {
        outTotal += element.amount * element.price;
    });

    const outHTML = document.getElementById('out');
    let htmlOut = `Total de Saídas: ${outTotal.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' })}`
    outHTML.innerHTML = htmlOut;

    let saldo = inTotal - outTotal;
    const saldoHtml = document.getElementById('saldo');
    let htmlSaldo = `Saldo: ${saldo.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' })}`
...
function filter() {
    ...
    const filterTable = data.filter(element => {
        if (searchSelect && searchSelect === element.type) {
            return true;
        }
        return false;
    });
